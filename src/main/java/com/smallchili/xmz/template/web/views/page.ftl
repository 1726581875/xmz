<template>
    <div>
                ...
                <el-table-column label="操作" width="180" align="center">
                    <template slot-scope="scope">
                        <el-button
                                type="text"
                                icon="el-icon-edit"
                                @click="handleEdit(scope.<#noparse>$</#noparse>index, scope.row)"
                        >编辑
                        </el-button>
                        <el-button
                                type="text"
                                icon="el-icon-delete"
                                class="red"
                                @click="handleDelete(scope.<#noparse>$</#noparse>index, scope.row.id)"
                        >删除
                        </el-button>
                    </template>
                </el-table-column>
        ...
        <el-dialog :title="dialogTitle" :visible.sync="editVisible" width="40%">
            <el-form ref="${domain}" :model="${domain}" label-width="70px">
                <#list fieldList as field>
                    <#if field.keyType!='PRI'>
...
<script>
  export default {
    ...
    methods: {
       ...
       */
      list() {
        this.<#noparse>$</#noparse>axios.get('http://localhost:9001/admin/${domain}s/list', {
          params: this.queryParam
        }).then(res => {
          let result = res.data;
          if (result.success) {
            this.${domain}List = result.data.content;
       ...
       */
      deleteById(id){
        this.<#noparse>$</#noparse>axios.delete("http://localhost:9001/admin/${domain}s/" + id)
          .then(res => {
      ...
      deleteSelected() {
        ...
        const ${domain}IdList = [];
        // 名字 list
        let del${Domain}Names = [];
        this.multipleSelection.forEach((${domain}, index) => {
          // 拿到要删除的${domain}Id
          ${domain}IdList.push(${domain}.id);
          // 取前五个名字
          if (index <= 5) {
            del${Domain}Names.push(${domain}.name);
          }
        });
        let delCount = ${domain}IdList.length;
        // 批量删除的名字拼接 String
        let delMsgStr = delCount <= 5 ? del${Domain}Names.toString() : del${Domain}Names.toString() + '...';

        //2、弹出框，提示信息,二次确认
        this.<#noparse>$</#noparse>confirm(`确定要批量删除 [<span style="color: red"><#noparse>$</#noparse>{delMsgStr}</span>] 一共
         <span style="color: red"><#noparse>$</#noparse>{${domain}IdList.length}</span> 列吗?`, '提示', {
          ...
          //3、发送批量删除请求
          this.<#noparse>$</#noparse>axios.post("http://localhost:9001/admin/${domain}s/batch/delete", ${domain}IdList)
            .then(res => {
      ...
      handleEdit(index, row) {
        ...
        let new${Domain} = JSON.parse(JSON.stringify(row));
      ...
      save() {
        ...
        //3、发请求
        this.<#noparse>$</#noparse>axios.post('http://localhost:9001/admin/${domain}s/${domain}', this.${domain})
          .then(res => {
            if (res.data.success) {
